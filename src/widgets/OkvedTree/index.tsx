...
export const OkvedTree = () => {
  ...
  const loadChildren = async (parentCode: string, level: number) => {
    try {
      const { data } = await $api.get<OkvedItem[]>(`/okved?level=${level}&parentCode=${parentCode}`);
      return data.map((item) => ({
        title: `${item.code} - ${item.description}`,
        key: item.code,
        isLeaf: level >= 3, // Предполагаем, что максимальный уровень - 3
      }));
    } catch (error) {
